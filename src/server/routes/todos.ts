...
/**
 * Build the partial update payload for a todo from validated input.
 * Extracted from the PUT handler to keep its cognitive complexity low.
 */
function buildTodoUpdate(validated: UpdateTodo): Partial<NewTodo> {
  const updateData: Partial<NewTodo> = { updatedAt: new Date().getTime() };
