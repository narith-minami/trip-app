...
import type { NewTrip } from "../db";
import { getDb, tripMembers, trips } from "../db";
import type { AuthContext } from "../middleware/auth";
import { getUserId, requireSession } from "../middleware/auth";

// Body schema for trip updates (id comes from the route param)
const UpdateTripBodySchema = UpdateTripSchema.omit({ id: true });
type UpdateTripBody = z.infer<typeof UpdateTripBodySchema>;

/**
 * Build the partial update payload for a trip from validated input.
 * Extracted from the PUT handler to keep its cognitive complexity low.
 */
function buildTripUpdate(validated: UpdateTripBody): Partial<NewTrip> {
  const updateData: Partial<NewTrip> = { updatedAt: new Date().getTime() };
...
const tripsRouter = new Hono<AuthContext>()
  .get("/", requireSession(), async (c) => {
    try {
      ...
      if (!userId) {
        return c.json({ error: HTTP_ERRORS.UNAUTHORIZED }, 401);
      }
      ...
      return c.json({ error: HTTP_ERRORS.INTERNAL }, 500);
    }
  })
  /**
   * POST /api/trips
  ...
  .get("/:tripId", requireSession(), async (c) => {
    try {
      ...
      if (!tripId) {
        return c.json({ error: HTTP_ERRORS.TRIP_ID_REQUIRED }, 400);
      }
      const db = getDb(c.env.DB);
      ...
      if (!member) {
        return c.json({ error: HTTP_ERRORS.FORBIDDEN }, 403);
      }
   ...
   * Update trip
   */
  .put("/:tripId", requireSession(), zValidator("json", UpdateTripBodySchema), async (c) => {
    try {
      const userId = getUserId(c);
      if (!userId) {
        return c.json({ error: HTTP_ERRORS.UNAUTHORIZED }, 401);
