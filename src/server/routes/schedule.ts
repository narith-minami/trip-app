...
import type { NewScheduleItem } from "../db";
...
/**
 * Build the partial update payload for a schedule item from validated input.
...
function buildScheduleUpdate(
  ...
  userId: string | null
): Partial<NewScheduleItem> {
  const updateData: Partial<NewScheduleItem> = {
    updatedBy: userId,
