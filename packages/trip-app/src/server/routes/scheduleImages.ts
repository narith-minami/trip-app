 ...
 * schedule item response (see `withImages` in ./schedule.ts), so there's no
 * GET here. Mounted under `/api/trips/:tripId/schedule/:itemId/images` so
  ...
  .delete("/:imageId", requireSession(), requireMember, async (c) => {
    try {
      ...
      const image = await db.query.scheduleItemImages.findFirst({
        where: and(
          eq(scheduleItemImages.id, imageId),
          eq(scheduleItemImages.scheduleItemId, itemId)
        ),
      });
      if (!image) {
