...
const commentsRouter = new Hono<TripMemberContext>()
  ...
  .post(
    ...
    async (c) => {
      try {
        ...
        console.error("POST /api/trips/:tripId/todos/:todoId/comments failed", error);
  ...
  .delete("/:commentId", requireSession(), requireMember, async (c) => {
    try {
      ...
      console.error("DELETE /api/trips/:tripId/todos/:todoId/comments/:commentId failed", error);
      return c.json({ error: ERR_INTERNAL }, 500);
    }
  });
